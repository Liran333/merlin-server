...
func (c *privacyCheck) check(ctx *gin.Context, user primitive.Account) {
	...
	if err != nil {
		commonctl.SendError(ctx, allerror.New(allerror.ErrorFailGetUserInfoWhenCheckPrivacyAgreement, "failed to get user info when checking privacy agreement", err))
