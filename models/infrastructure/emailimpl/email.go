...
func (impl emailImpl) Send(modelName, content, user, url string) error {
	body := impl.GetEmailTemplate(modelName, content, user, url)
	return impl.instance.Send(impl.receiver, "Model", body)
}

func (impl emailImpl) GetEmailTemplate(name, reason, user, url string) string {
	return fmt.Sprintf(impl.MailTemplate, "模型", name, reason, user, url)
}
